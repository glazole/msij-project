...
---
![alt text](MinIO_Spark_Iceberg_Jupyter.png)
...
📌 Все сервисы объединены в одну сеть для хранения данных и логов определены тома `minio-data` и `spark-logs` - это упрощает локальную отладку, а также позволяет гибко настраивать пути `s3a://` и Spark Session изнутри Jupyter или .py-скриптов.
